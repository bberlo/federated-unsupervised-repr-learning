...
for x in args.test_files:
	gyr_input, acc_input, labels = \
		np.load('datasets/HHAR_Data/gyr_seg_' + str(x) + '.npy').astype(np.float32), \
		np.load('datasets/HHAR_Data/acc_seg_' + str(x) + '.npy').astype(np.float32), \
		np.load('datasets/HHAR_Data/lb_' + str(x) + '.npy')
...
y_pred = act_model.predict([acc_test, gyr_test], batch_size=10)
