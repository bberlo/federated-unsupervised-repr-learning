...
# Model data
ae_train_data, acc_train, gyr_train, ae_train_labels, acc_eval, gyr_eval, label_eval = testScriptDataFetchFunctions.get_har_data(
	args.train_files,
	args.clients,
	'unsupervised',
...
with fedAvgServer.FedAvgServer([ae_model, instructions]) as server:
	for comm_round_checkpoint in range(0, args.rounds, instructions["communication_rounds"]):
		_, _ = server.simulate_train_model(ae_train_data)

		# Custom early stopping procedure
		decoded_pred = server.plan[0].predict([acc_eval, gyr_eval], batch_size=instructions["batch_size"])
		current_mse = floor(sum([np.mean(np.square(np.subtract(x, y))) for x, y in zip(decoded_pred, [acc_eval, gyr_eval])]) * 10000) / 10000.0

		patience_overflow = ae_es_object.update(current_mse, server.plan[0])
		if patience_overflow:
			break

	path, epochs = ae_es_object.return_best_version(server.plan[0])
...
o = keras.layers.Dense(ae_train_labels.shape[1], activation='softmax')(x)
...
for comm_round_checkpoint in range(0, args.rounds, instructions["communication_rounds"]):
	act_model.fit(backbone_bottleneck_f, ae_train_labels, batch_size=instructions["batch_size"], epochs=instructions["epochs"], verbose=2)
